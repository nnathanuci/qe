...
public:
	...
	RC remove_entry_by_page(Entry key);
	...
	RC remove(int value);
	RC remove(float key);
	RC tree_remove(unsigned int nodePointer, Entry key, int& remove_i_e, unsigned int& remove_left);
	RC tree_remove_real(unsigned int nodePointer, Entry key, int& remove_i_e, unsigned int& remove_left);
